...
create table emprestimo (
	...
	constraint fk_ept_idaluno foreign key (idaluno) references aluno (idaluno)
);

insert into emprestimo (idaluno, data_emprestimo, data_devolucao, valor, devolvido) values (1, '2021-05-02', '2012-05-12', 10, 'S');
insert into emprestimo (idaluno, data_emprestimo, data_devolucao, valor, devolvido) values (1, '2012-04-23', '2012-05-03', 5, 'N');
insert into emprestimo (idaluno, data_emprestimo, data_devolucao, valor, devolvido) values (2, '2012-05-20', '2012-05-20', 12, 'N');
insert into emprestimo (idaluno, data_emprestimo, data_devolucao, valor, devolvido) values (3, '2012-05-20', '2012-05-20', 8, 'S');
insert into emprestimo (idaluno, data_emprestimo, data_devolucao, valor, devolvido) values (4, '2012-05-05', '2012-05-15', 15, 'N');
insert into emprestimo (idaluno, data_emprestimo, data_devolucao, valor, devolvido) values (4, '2012-05-17', '2012-05-17', 20, 'S');
insert into emprestimo (idaluno, data_emprestimo, data_devolucao, valor, devolvido) values (4, '2012-05-18', '2012-05-18', 5, 'S');
...
create index idx_ept_emprestimo on emprestimo (data_emprestimo);
create index idx_ept_devolucao on emprestimo (data_devolucao);
...
select round(avg(valor), 1) from emprestimo;
...
select count(idemprestimo) from emprestimo where data_emprestimo between '2012-05-05' and '2012-05-10'

-- CONSULTAS COM JOIN
	-- O nome do livro, a categoria e a editora (LIVRO) – fazer uma view
create view livro_informacoes as
select
	lvr.nome as livro,
...
from
	emprestimo ept
left outer join
	aluno aln on ept.idaluno = aln.idaluno
...
left outer join
	livro lvr on epl.idlivro = lvr.idlivro
...
group by
	...
	-- O nome do aluno e a quantidade de empréstimo de cada aluno (EMPRESTIMO).
select
	aln.nome as aluno,
	count(idemprestimo)
from
...
where extract(year from data_emprestimo) = 2012 and extract(month from data_emprestimo) = 04
...
select 
	nome, substring(nome from 5 for 10) from autor
...
select
	ept.data_emprestimo,
	ept.valor,
	(select count(elv.idemprestimo) from emprestimo_livro elv where elv.idemprestimo = ept.idemprestimo)
from
	emprestimo ept
where
		(select
		...
		from emprestimo_livro elv 
			where elv.idemprestimo = ept.idemprestimo) > 1
